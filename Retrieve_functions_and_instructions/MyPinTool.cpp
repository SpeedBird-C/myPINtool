...
KNOB< string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "rokTEST2ARG3withShellcode.out", "trace file");
     ...
     else if (INS_IsDirectControlFlow(ins))
     {
               ...
               )
          {
               ...
               INS_InsertCall(ins, IPOINT_BEFORE, AFUNPTR(EmitDirectCall), IARG_THREAD_ID, IARG_PTR, new string(s), IARG_BOOL,
                    tailcall, IARG_FUNCARG_CALLSITE_VALUE, 0, IARG_FUNCARG_CALLSITE_VALUE, 1, IARG_FUNCARG_CALLSITE_VALUE, 2, IARG_END);
          }
     }
...
VOID Trace(TRACE trace, VOID* v)
{
     ...
     if (enabled)
     {
          for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
          {
               ...
               out << "*****" << "Adress " << hex << BBL_Address(bbl) << "*****" << endl;
               out << hex << dec;
